' JQ STAGE 4 - WORLD GOLD ROBUSTNESS - EViews only
' Standalone embedded-data version. No Python. No graphs.
' Minimal output to avoid EViews crash.
' Output: JQ_STAGE4_XAU_MINIMAL.rtf and JQ_STAGE4_XAU_MINIMAL.wf1
' Both files are saved beside this PRG file.
close @all

' Set the working directory to the folder containing this PRG file.
%stage4_path = @runpath
cd %stage4_path
wfcreate(wf=JQXAU) u 122
alpha month_jalali
series tedpix
series usd
series goldcoin
...
series LXAU = log(xau)
series LCPREM = LGC - LUSD - LXAU

series DLUSD = d(LUSD)
series DLGC = d(LGC)
series DLXAU = d(LXAU)
series DLVEH = d(LVEH)
series DLHOUS = d(LHOUS)

' Preferred world-gold robustness system:
' domestic dollar, domestic gold coin, world gold price, vehicle, housing.
' This checks whether the four-asset long-run result is only absorbing world gold movements.
group GXAU5 LUSD LGC LXAU LVEH LHOUS
group DXAU5 DLUSD DLGC DLXAU DLVEH DLHOUS

' ---------------------------------------------------------------------
' 3. MINIMAL OUTPUTS
' ---------------------------------------------------------------------
freeze(T1) LXAU.stats
freeze(T2) LCPREM.stats
freeze(T3) DXAU5.cor

var VLAG.ls 1 4 LUSD LGC LXAU LVEH LHOUS
freeze(T4) VLAG.laglen(4)

' Case 3 Johansen/VECM, VAR(2) equivalent: one lagged first difference.
var VXAU.ec(c,1) 1 1 LUSD LGC LXAU LVEH LHOUS
freeze(T5) VXAU.coint(rank)
freeze(T6) VXAU.output

' Alternative decomposition system using coin premium instead of coin price.
' Use only as an auxiliary robustness check; if LCPREM behaves as I(0), report cautiously.
group GPREM LUSD LXAU LCPREM LVEH LHOUS
freeze(T7) GPREM.stats
var VPREM.ec(c,1) 1 1 LUSD LXAU LCPREM LVEH LHOUS
freeze(T8) VPREM.coint(rank)
freeze(T9) VPREM.output
...
S4.save(t=rtf) "JQ_STAGE4_XAU_MINIMAL.rtf"
wfsave "JQ_STAGE4_XAU_MINIMAL.wf1"
