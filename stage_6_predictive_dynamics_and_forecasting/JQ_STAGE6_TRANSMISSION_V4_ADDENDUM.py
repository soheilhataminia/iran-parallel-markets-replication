"""
=====================================================================================
 STAGE 6 - V4 ADDENDUM (reporting round). The locked core JQ_STAGE6_TRANSMISSION_V4
 is unchanged; this addendum only adds:

  A1  Holm-adjusted p-values WITHIN each robustness variant's own six-cell family
      (fully-recursive shocks; TAU0 = 60; TAU0 = 84), so that robustness claims are
      stated at the family level. Expected per referee: 0.0377 / 0.0407 / 0.1021.
  A2  OOS episode-exclusion robustness of predictive mapping and evaluation,
      with full-history vintage shock construction:
      for each Stage-1 episode (1397/02, 1399/04, 1401/06, 1404/03; window +/-2),
      the OOS exercise is re-run with (i) training rows whose information set or
      target window [t-2, t+h] touches the episode removed from the predictive
      regressions (symmetric with the origin rule), and (ii) evaluation origins
      tau whose window [tau-2, tau+h] touches the episode removed from
      evaluation. The vintage VECM / shock system retains the full history
      (months cannot be deleted from a recursive system). The earlier
      LP-row-exclusion exercise is retained in the core under its correct label:
      in-sample robustness of the month-1 LP coefficient.

 Dependency: this addendum REQUIRES the locked core output S6v4_results.json
 (produced by JQ_STAGE6_TRANSMISSION_V4.py). Pass the output directory as the
 first argument; optionally pass the core-JSON path explicitly as the second.

 Self-contained: data embedded; numpy/scipy only.
=====================================================================================
"""
...
CORE_JSON = sys.argv[2] if len(sys.argv) > 2 else os.path.join(OUT, "S6v4_results.json")
if not os.path.exists(CORE_JSON):
    raise SystemExit("This addendum requires the locked core output S6v4_results.json "
                     "(run JQ_STAGE6_TRANSMISSION_V4.py first); place it in the output "
                     "directory or pass its path as the second argument.")
...
Y = np.column_stack([np.log(col("usd")), np.log(col("goldcoin")),
                     np.log(col("vehicle")), np.log(col("housing"))])
LXAU = np.log(col("xau"))
T, K = Y.shape
DY = np.diff(Y, axis=0); dX = np.diff(LXAU)
iU, iG, iV, iH = 0, 1, 2, 3
OUTC = {"HOUS": 3, "VEH": 2}
...
def johansen(Ylev):
    ...
    return dict(beta=beta, alpha=Bf[0:1].T, Gamma1=Bf[1:1 + K].T, c=Bf[-1], U=U)
...
def prem_equation(Ylev, dXs, nlags=2):
    ...
    b, e, XtXi, nn, kk = ols(R[nlags:, 1], np.column_stack(Xl))
    return v, dict(resid=e, b=b, sd=float(e.std(ddof=1)))

def vintage(tauminus1):
    Yv = Y[:tauminus1]; dXv = dX[:tauminus1 - 1]
    vv, prv = prem_equation(Yv, dXv, nlags=2)
    return vv, prv, float(vv["U"][:, iU].std(ddof=1))
...
TAU0, HFC = 72, [1, 3, 6]
def run_family(excl=None):
    """excl: set of months (episode window). Returns {cell: (R2_M2, CW_p)}."""
    out = {}
    for oc, ic in OUTC.items():
        for h in HFC:
            E0, E2, Y0p, Y2p = [], [], [], []
            for tau in range(TAU0, T - h + 1):
                if excl and any((tau - 2 <= m <= tau + h) for m in excl):
                    continue
                ...
                for t in range(4, tau - h + 1):
                    if excl and any((t - 2 <= m <= t + h) for m in excl):
                        continue
                    ya = Y[t - 1 + h, ic] - Y[t - 1, ic]
                    base = [1.0, DY[t - 2, ic], float((Y[t - 1] @ bt)[0]), dX[t - 2]]
                    rows_y.append(ya); r0.append(base)
                    r2.append(base + [sUo[t - 4], sCo[t - 4]])
                rows_y = np.array(rows_y)
                suT, scT = rt_shocks_at(vv, prv, sdUv, tau)
                baseT = [1.0, DY[tau - 2, ic], float((Y[tau - 1] @ bt)[0]), dX[tau - 2]]
                x0 = np.array(baseT); x2 = np.array(baseT + [suT, scT])
                ya = Y[tau - 1 + h, ic] - Y[tau - 1, ic]
                b0 = linalg.lstsq(np.array(r0), rows_y)[0]
                b2 = linalg.lstsq(np.array(r2), rows_y)[0]
                yh0, yh2 = float(x0 @ b0), float(x2 @ b2)
                Y0p.append(yh0); Y2p.append(yh2); E0.append(ya - yh0); E2.append(ya - yh2)
            sse0 = float((np.array(E0) ** 2).sum())
            out[f"{oc}_h{h}"] = dict(
                n=len(E0), R2_M2=float(1 - (np.array(E2) ** 2).sum() / sse0),
                CW_p=clark_west(E0, E2, Y0p, Y2p, bw=h + 1)[1])
    return out

resA = {}

# A1: Holm within each robustness variant's family (from the locked V4 JSON)
core = json.load(open(CORE_JSON))
fam_rec = {f"{oc}_h{h}": core["pseudo_oos_recursive_robustness"][oc][f"h{h}"]["CW_M2vsM0"][1]
           for oc in OUTC for h in HFC}
resA["holm_within_variant_families"] = {
    "fully_recursive": dict(p=fam_rec, holm6=holm_dict(fam_rec)),
    "TAU0_60": dict(p={k: v["CW_p"] for k, v in core["window_sensitivity"]["TAU0_60"].items()},
                    holm6=holm_dict({k: v["CW_p"] for k, v in core["window_sensitivity"]["TAU0_60"].items()})),
    "TAU0_84": dict(p={k: v["CW_p"] for k, v in core["window_sensitivity"]["TAU0_84"].items()},
                    holm6=holm_dict({k: v["CW_p"] for k, v in core["window_sensitivity"]["TAU0_84"].items()})),
}

# A2: OOS episode-exclusion robustness of predictive mapping and evaluation,
# with full-history vintage shock construction
episodes = {"1397/02": 26, "1399/04": 52, "1401/06": 78, "1404/03": 111}
resA["oos_episode_exclusion_robustness"] = {}
resA["oos_episode_exclusion_robustness_label"] = ("OOS episode-exclusion robustness of predictive mapping and evaluation, with full-history vintage shock construction")
for nm, m0 in episodes.items():
    W = set(range(m0 - 2, m0 + 3))
    fam = run_family(excl=W)
    ps = {k: v["CW_p"] for k, v in fam.items()}
    resA["oos_episode_exclusion_robustness"][nm] = dict(
        family=fam, holm6=holm_dict(ps),
        HOUS_h1=dict(R2_M2=fam["HOUS_h1"]["R2_M2"], CW_p=fam["HOUS_h1"]["CW_p"],
                     Holm=holm_dict(ps)["HOUS_h1"], n=fam["HOUS_h1"]["n"]))

# sanity: no-exclusion run must reproduce the locked cell
fam0 = run_family(excl=None)
resA["sanity_no_exclusion"] = dict(HOUS_h1=fam0["HOUS_h1"],
                                   matches_core=abs(fam0["HOUS_h1"]["R2_M2"] -
                                                    core["pseudo_oos_main"]["HOUS"]["h1"]["R2_M2"]) < 1e-10)

with open(os.path.join(OUT, "S6v4_addendum_results.json"), "w") as f:
    json.dump(resA, f, indent=2, default=float)

print("=== A1: Holm WITHIN robustness families (HOUS_h1 cell) ===")
for k, v in resA["holm_within_variant_families"].items():
    print("  %-16s raw p = %.4f   Holm6 = %.4f" % (k, v["p"]["HOUS_h1"], v["holm6"]["HOUS_h1"]))
print("\n=== A2: OOS episode-exclusion robustness of predictive mapping and evaluation,")
print("        with full-history vintage shock construction (HOUS_h1) ===")
for nm, v in resA["oos_episode_exclusion_robustness"].items():
    print("  drop %-8s n=%2d  R2=%.4f  CW p=%.4f  Holm6=%.4f" %
          (nm, v["HOUS_h1"]["n"], v["HOUS_h1"]["R2_M2"], v["HOUS_h1"]["CW_p"], v["HOUS_h1"]["Holm"]))
print("\nsanity (no exclusion reproduces locked core):", resA["sanity_no_exclusion"]["matches_core"])
