'====================================================================================
' STAGE 1 - FINAL Baseline nominal Jalali monthly VECM pipeline WITH EViews ADF - V2
' EMBEDDED-DATA VERSION with ADF via series.uroot object commands, IRF/FEVD and diagnostics.
...
'------------------------------------------------------------------------------------
' 3. DESCRIPTIVES AND FIGURES
'------------------------------------------------------------------------------------
freeze(T01_DESC_LEVELS)  G5.stats
freeze(T02_DESC_RETURNS) DG5.stats
freeze(T03_CORR_RET)     DG5.cor
freeze(F01_CUMLOG)       G5.line
freeze(F02_RETURNS)      DG5.line

'------------------------------------------------------------------------------------
' 4. ADF UNIT ROOT TESTS IN EVIEWS
'------------------------------------------------------------------------------------
' ADF is run once for each baseline nominal series.
' Three specifications are reported for each variable:
'   (i)  log level with intercept, AIC lag selection;
'   (ii) log level with intercept and trend, AIC lag selection;
'   (iii) first difference with intercept, AIC lag selection.
' Zivot-Andrews / one-break tests remain in Python because they are part of the structural-break audit.
' EViews command syntax follows the official UROOT command form:
'   uroot(adf,exog=...,dif=...,lagmethod=aic,maxlag=12) series_name

' Levels: intercept only
freeze(T04_ADF_LUSD_C)      LUSD.uroot(adf,exog=const,dif=0,lagmethod=aic,maxlag=12)
freeze(T05_ADF_LGCOIN_C)    LGCOIN.uroot(adf,exog=const,dif=0,lagmethod=aic,maxlag=12)
freeze(T06_ADF_LSTOCK_C)    LSTOCK.uroot(adf,exog=const,dif=0,lagmethod=aic,maxlag=12)
freeze(T07_ADF_LVEHICLE_C)  LVEHICLE.uroot(adf,exog=const,dif=0,lagmethod=aic,maxlag=12)
freeze(T08_ADF_LHOUSING_C)  LHOUSING.uroot(adf,exog=const,dif=0,lagmethod=aic,maxlag=12)

' Levels: intercept and trend
freeze(T09_ADF_LUSD_CT)     LUSD.uroot(adf,exog=trend,dif=0,lagmethod=aic,maxlag=12)
freeze(T10_ADF_LGCOIN_CT)   LGCOIN.uroot(adf,exog=trend,dif=0,lagmethod=aic,maxlag=12)
freeze(T11_ADF_LSTOCK_CT)   LSTOCK.uroot(adf,exog=trend,dif=0,lagmethod=aic,maxlag=12)
freeze(T12_ADF_LVEHICLE_CT) LVEHICLE.uroot(adf,exog=trend,dif=0,lagmethod=aic,maxlag=12)
freeze(T13_ADF_LHOUSING_CT) LHOUSING.uroot(adf,exog=trend,dif=0,lagmethod=aic,maxlag=12)

' First differences: intercept only
freeze(T14_ADF_DLUSD_C)     LUSD.uroot(adf,exog=const,dif=1,lagmethod=aic,maxlag=12)
freeze(T15_ADF_DLGCOIN_C)   LGCOIN.uroot(adf,exog=const,dif=1,lagmethod=aic,maxlag=12)
freeze(T16_ADF_DLSTOCK_C)   LSTOCK.uroot(adf,exog=const,dif=1,lagmethod=aic,maxlag=12)
freeze(T17_ADF_DLVEHICLE_C) LVEHICLE.uroot(adf,exog=const,dif=1,lagmethod=aic,maxlag=12)
freeze(T18_ADF_DLHOUSING_C) LHOUSING.uroot(adf,exog=const,dif=1,lagmethod=aic,maxlag=12)

'------------------------------------------------------------------------------------
' 5. VAR LAG SELECTION
'------------------------------------------------------------------------------------
var VAR5_LAG.ls 1 4 G5
freeze(T19_VAR5_LAG_SELECT) VAR5_LAG.laglen(4)

var VAR4_LAG.ls 1 4 G4
freeze(T20_VAR4_LAG_SELECT) VAR4_LAG.laglen(4)

'------------------------------------------------------------------------------------
' 6. JOHANSEN RANK TESTS AND VECM ESTIMATION - MAIN lag interval 1 1
'------------------------------------------------------------------------------------
var VEC5_MAIN.ec(c,1) 1 1 LUSD LGCOIN LSTOCK LVEHICLE LHOUSING
freeze(T21_JOH_5_MAIN) VEC5_MAIN.coint(rank)
freeze(T22_VEC_5_MAIN) VEC5_MAIN.output
freeze(F03_IRF_5_MAIN)  VEC5_MAIN.impulse(36,m,se=boot,rep=500,cilevels=0.95) LUSD LGCOIN LSTOCK LVEHICLE LHOUSING @ LUSD LGCOIN LSTOCK LVEHICLE LHOUSING @ LUSD LGCOIN LSTOCK LVEHICLE LHOUSING
freeze(F04_FEVD_5_MAIN) VEC5_MAIN.decomp(36,g,imp=chol) LUSD LGCOIN LSTOCK LVEHICLE LHOUSING @ @ LUSD LGCOIN LSTOCK LVEHICLE LHOUSING
freeze(T22A_FEVD_5_MAIN_12) VEC5_MAIN.decomp(12,imp=chol) LUSD LGCOIN LSTOCK LVEHICLE LHOUSING @ @ LUSD LGCOIN LSTOCK LVEHICLE LHOUSING

var VEC4_MAIN.ec(c,1) 1 1 LUSD LGCOIN LVEHICLE LHOUSING
freeze(T23_JOH_4_MAIN) VEC4_MAIN.coint(rank)
freeze(T24_VEC_4_MAIN) VEC4_MAIN.output
freeze(F05_IRF_4_MAIN)  VEC4_MAIN.impulse(36,m,se=boot,rep=500,cilevels=0.95) LUSD LGCOIN LVEHICLE LHOUSING @ LUSD LGCOIN LVEHICLE LHOUSING @ LUSD LGCOIN LVEHICLE LHOUSING
freeze(F06_FEVD_4_MAIN) VEC4_MAIN.decomp(36,g,imp=chol) LUSD LGCOIN LVEHICLE LHOUSING @ @ LUSD LGCOIN LVEHICLE LHOUSING
freeze(T24A_FEVD_4_MAIN_12) VEC4_MAIN.decomp(12,imp=chol) LUSD LGCOIN LVEHICLE LHOUSING @ @ LUSD LGCOIN LVEHICLE LHOUSING

' Deterministic sensitivity
var VEC4_CASE_B.ec(b,1) 1 1 LUSD LGCOIN LVEHICLE LHOUSING
freeze(T25_JOH_4_CASE_B) VEC4_CASE_B.coint(rank)
var VEC4_CASE_D.ec(d,1) 1 1 LUSD LGCOIN LVEHICLE LHOUSING
freeze(T26_JOH_4_CASE_D) VEC4_CASE_D.coint(rank)

'------------------------------------------------------------------------------------
' 7. LAG ROBUSTNESS - lag interval 1 2
'------------------------------------------------------------------------------------
var VEC5_L12.ec(c,1) 1 2 LUSD LGCOIN LSTOCK LVEHICLE LHOUSING
freeze(T27_JOH_5_L12) VEC5_L12.coint(rank)
freeze(T28_VEC_5_L12) VEC5_L12.output

var VEC4_L12.ec(c,1) 1 2 LUSD LGCOIN LVEHICLE LHOUSING
freeze(T29_JOH_4_L12) VEC4_L12.coint(rank)
freeze(T30_VEC_4_L12) VEC4_L12.output

'------------------------------------------------------------------------------------
' 8. DIAGNOSTICS
'------------------------------------------------------------------------------------
' Main VEC(4) diagnostics added back to the EViews output file.
' These use EViews VAR/VEC command-line view names: ARLM, JBERA, WHITE, and ARROOTS.
freeze(T31_LM_4_MAIN)          VEC4_MAIN.arlm(12)
freeze(T32_NORMALITY_4)        VEC4_MAIN.jbera(factor=cor)
freeze(T33_WHITE_4)            VEC4_MAIN.white
freeze(T34_ROOTS_4_TABLE)      VEC4_MAIN.arroots
freeze(F08_ROOTS_4_GRAPH)      VEC4_MAIN.arroots(graph)

'------------------------------------------------------------------------------------
' 9. ECT INDEX AND RELATIVE-ADJUSTMENT REGRESSIONS
' Coefficients below are filled by the Python replication script; update them if re-running
' a materially different model. The supplied Python output contains the exact values.
'------------------------------------------------------------------------------------
' PLACEHOLDER coefficients are replaced in the generated PRG file.

series ECT4_RAW = LUSD -0.3299534582*LGCOIN +0.4875974964*LVEHICLE -1.0185084262*LHOUSING
scalar ECT4_MEAN = @mean(ECT4_RAW)
scalar ECT4_SD   = @stdev(ECT4_RAW)
series ECT4_STD  = (ECT4_RAW - ECT4_MEAN)/ECT4_SD
freeze(F07_ECT4_STD) ECT4_STD.line

series REL_3  = 100*(0.5*(LUSD(+3)-LUSD + LGCOIN(+3)-LGCOIN) - 0.5*(LVEHICLE(+3)-LVEHICLE + LHOUSING(+3)-LHOUSING))
series REL_6  = 100*(0.5*(LUSD(+6)-LUSD + LGCOIN(+6)-LGCOIN) - 0.5*(LVEHICLE(+6)-LVEHICLE + LHOUSING(+6)-LHOUSING))
series REL_12 = 100*(0.5*(LUSD(+12)-LUSD + LGCOIN(+12)-LGCOIN) - 0.5*(LVEHICLE(+12)-LVEHICLE + LHOUSING(+12)-LHOUSING))

equation EQ_REL3_HAC.ls(cov=hac,covkern=bart,covbw=3) REL_3 c ECT4_STD
equation EQ_REL6_HAC.ls(cov=hac,covkern=bart,covbw=6) REL_6 c ECT4_STD
equation EQ_REL12_HAC.ls(cov=hac,covkern=bart,covbw=12) REL_12 c ECT4_STD
freeze(T35_REL3_HAC) EQ_REL3_HAC.output
freeze(T36_REL6_HAC) EQ_REL6_HAC.output
freeze(T37_REL12_HAC) EQ_REL12_HAC.output

' Event dummies in Jalali months (approximate Gregorian events): 1397m02, 1399m04, 1401m06, 1404m03
series D1397M02 = 0
series D1399M04 = 0
series D1401M06 = 0
series D1404M03 = 0
...
equation EQ_REL3_EVENT.ls(cov=hac,covkern=bart,covbw=3) REL_3 c ECT4_STD D1397M02 D1399M04 D1401M06 D1404M03
equation EQ_REL6_EVENT.ls(cov=hac,covkern=bart,covbw=6) REL_6 c ECT4_STD D1397M02 D1399M04 D1401M06 D1404M03
equation EQ_REL12_EVENT.ls(cov=hac,covkern=bart,covbw=12) REL_12 c ECT4_STD D1397M02 D1399M04 D1401M06
freeze(T38_REL3_EVENT) EQ_REL3_EVENT.output
freeze(T39_REL6_EVENT) EQ_REL6_EVENT.output
freeze(T40_REL12_EVENT) EQ_REL12_EVENT.output

' Save one Word-compatible file containing all frozen tables and figures only.
' This does not change any estimation, test, table, or figure; it only controls export.
spool SP_JALALI_Q1_FINAL
SP_JALALI_Q1_FINAL.append T01_DESC_LEVELS
SP_JALALI_Q1_FINAL.append T02_DESC_RETURNS
SP_JALALI_Q1_FINAL.append T03_CORR_RET
SP_JALALI_Q1_FINAL.append F01_CUMLOG
SP_JALALI_Q1_FINAL.append F02_RETURNS
SP_JALALI_Q1_FINAL.append T04_ADF_LUSD_C
SP_JALALI_Q1_FINAL.append T05_ADF_LGCOIN_C
SP_JALALI_Q1_FINAL.append T06_ADF_LSTOCK_C
SP_JALALI_Q1_FINAL.append T07_ADF_LVEHICLE_C
SP_JALALI_Q1_FINAL.append T08_ADF_LHOUSING_C
SP_JALALI_Q1_FINAL.append T09_ADF_LUSD_CT
SP_JALALI_Q1_FINAL.append T10_ADF_LGCOIN_CT
SP_JALALI_Q1_FINAL.append T11_ADF_LSTOCK_CT
SP_JALALI_Q1_FINAL.append T12_ADF_LVEHICLE_CT
SP_JALALI_Q1_FINAL.append T13_ADF_LHOUSING_CT
SP_JALALI_Q1_FINAL.append T14_ADF_DLUSD_C
SP_JALALI_Q1_FINAL.append T15_ADF_DLGCOIN_C
SP_JALALI_Q1_FINAL.append T16_ADF_DLSTOCK_C
SP_JALALI_Q1_FINAL.append T17_ADF_DLVEHICLE_C
SP_JALALI_Q1_FINAL.append T18_ADF_DLHOUSING_C
SP_JALALI_Q1_FINAL.append T19_VAR5_LAG_SELECT
SP_JALALI_Q1_FINAL.append T20_VAR4_LAG_SELECT
SP_JALALI_Q1_FINAL.append T21_JOH_5_MAIN
SP_JALALI_Q1_FINAL.append T22_VEC_5_MAIN
SP_JALALI_Q1_FINAL.append F03_IRF_5_MAIN
SP_JALALI_Q1_FINAL.append F04_FEVD_5_MAIN
SP_JALALI_Q1_FINAL.append T22A_FEVD_5_MAIN_12
SP_JALALI_Q1_FINAL.append T23_JOH_4_MAIN
SP_JALALI_Q1_FINAL.append T24_VEC_4_MAIN
SP_JALALI_Q1_FINAL.append F05_IRF_4_MAIN
SP_JALALI_Q1_FINAL.append F06_FEVD_4_MAIN
SP_JALALI_Q1_FINAL.append T24A_FEVD_4_MAIN_12
SP_JALALI_Q1_FINAL.append T25_JOH_4_CASE_B
SP_JALALI_Q1_FINAL.append T26_JOH_4_CASE_D
SP_JALALI_Q1_FINAL.append T27_JOH_5_L12
SP_JALALI_Q1_FINAL.append T28_VEC_5_L12
SP_JALALI_Q1_FINAL.append T29_JOH_4_L12
SP_JALALI_Q1_FINAL.append T30_VEC_4_L12
SP_JALALI_Q1_FINAL.append T31_LM_4_MAIN
SP_JALALI_Q1_FINAL.append T32_NORMALITY_4
SP_JALALI_Q1_FINAL.append T33_WHITE_4
SP_JALALI_Q1_FINAL.append T34_ROOTS_4_TABLE
SP_JALALI_Q1_FINAL.append F08_ROOTS_4_GRAPH
SP_JALALI_Q1_FINAL.append F07_ECT4_STD
SP_JALALI_Q1_FINAL.append T35_REL3_HAC
SP_JALALI_Q1_FINAL.append T36_REL6_HAC
SP_JALALI_Q1_FINAL.append T37_REL12_HAC
SP_JALALI_Q1_FINAL.append T38_REL3_EVENT
SP_JALALI_Q1_FINAL.append T39_REL6_EVENT
SP_JALALI_Q1_FINAL.append T40_REL12_EVENT

' EViews exports Word-compatible documents as RTF. Open this file in Microsoft Word and, if needed, Save As DOCX.
' Save the RTF and the complete EViews workfile beside this PRG file.
SP_JALALI_Q1_FINAL.save(t=rtf) "JQ_STAGE1_MAIN_NOMINAL_FINAL_WITH_ADF_V2.rtf"
wfsave "JQ_STAGE1_MAIN_NOMINAL_FINAL_WITH_ADF_V2.wf1"

'============================================================
' END: Stage 1 outputs are saved beside the running PRG file.
'============================================================
